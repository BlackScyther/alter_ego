...
console.log('phase patch', phasePatch.phase === 'initiative' ? 'ok' : phasePatch);

const rewards = await fetch(
  `${base}/api/campaigns/${created.campaignId}/characters/${char.id}/rewards`,
  {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({ xp: 50, goldGp: 10 })
  }
).then((r) => r.json());
console.log('rewards', rewards.ok ? 'ok' : rewards);
