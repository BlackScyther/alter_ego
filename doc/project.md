...
| **Developer / host** | Default deploy target: `server.masche.at` (Coolify + Docker), live at `https://alter-ego.apps.masche.at` — use the `server-masche-deploy` skill for host ops, [deploy-vps.md](deploy-vps.md) for Alter-Ego specifics; shared-hosting alternative in [deploy-online.md](deploy-online.md); locally `npm run dev:all` |
...
- **Race step:** core/subrace pickers (the subrace picker lists a "None (play the base race)" option first, so a race that has subraces can be played without one — subraces are optional), ability “or” choices on race step, racial power/feat grant tiles, compendium hover links in preview and notes; descriptive racial traits (languages, skill bonus, resistances, senses, speed, other texts) grouped in one collapsed-by-default "Racial Traits" fold; structured traits applied to the sheet/mirror at their fields (Languages, Resistances, Special Senses, Speed) with manual edits preserved; racial powers that offer a choice of attack ability and/or damage type (e.g. Dragon Breath) show dropdowns inside that power's grant fold and store the picks per character (`selections.racePowerAbilityChoices` / `selections.racePowerDamageChoices`); when a power offers these choices they are required to finish the race step and annotate the power name in the notes (e.g. `Dragon Breath (Dexterity, fire)`); when a subrace replaces a base-race trait or granted power/feat (e.g. a Draconian subrace's flight replacing a Dragonborn trait), the base item is suppressed from the combined preview, notes, grant tiles, and sheet so only the subrace's version shows (detected by auto-parsing the subrace body plus a curated override map); monster races that carry no flavor in their race entry (e.g. Bugbear) get supplemental **lore-only** folds pulled from the matching Monster compendium entry (statistics stripped), shown collapsed in the preview and resolved by exact race-name → monster-name match plus a curated override map (`metadata/race-monster-fluff.json`)
...
The compendium is moving from runtime HTML parsing to a **normalized SQLite** schema built once at import time. `tools/normalize/normalize.mjs` (`npm run normalize`) reads the denormalized `entries` table and writes structured tables (races/subraces incl. structured racial traits — languages, resistances, senses, free-text traits, parsed speed, subrace replacement rows (`race_replacement`), classes incl. hybrid links, powers with exact types plus per-power ability-choice and damage-type options, equipment stats incl. per-level `item_level` tiers for level-scaled items, source books with release dates) into the same DB file, reusing the existing parsers and recording parse failures in `normalize_warnings`. The app reads normalized-first with a parse fallback per category, so the migration is incremental and non-breaking. The GM can edit source-book release dates and filter compendium pickers by publication date (Workshop "Source dates" + the duplicate picker's "released on/before" control). Details: [compendium-schema.md](compendium-schema.md).
...
| [deploy-vps.md](deploy-vps.md) | Ubuntu 26.04 + Coolify single-container deploy |
